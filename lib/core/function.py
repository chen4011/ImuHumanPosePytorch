...
def validate(config, loader, dataset, model, criterion, output_dir,
    ...
    with torch.no_grad():
        ...
        for i, (input_, meta_) in enumerate(loader):
            # print('fuck you research')
            ...
            if i % config.PRINT_FREQ == 0:
                ...
                # save_debug_images(config, input, meta_for_debug_imgs, target,
                #                   pre * 4, extra['origin_hms'], prefix)
                save_debug_heatmaps(config, input, None, None, None, extra['origin_hms'], prefix)
                if 'fused_hms' in extra:
                    ...
                    save_debug_heatmaps(config, input, None, None, None, fused_hms, prefix)
