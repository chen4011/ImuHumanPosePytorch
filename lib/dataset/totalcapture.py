...
class TotalCaptureDataset(JointsDataset):
    ...
    def __init__(self, cfg, image_set, is_train, transform=None, selected_cam=None):
        ...
        if osp.isfile(grouping_db_pickle_file):
            ...
            # with open(osp.join(self.root, 'totalcapture', 'quickload',
            #                                'totalcapture_quickload_{}_cam_{}.txt'
            #                                .format(image_set, selected_cam_string)), 'wb') as f:
            #     pickle.dump(grouping_db_to_dump, f)
        if self.is_train:
            self.grouping = self.grouping[::1]
        else:
            self.grouping = self.grouping[::1] # 每一個都取
            # print('=>grouping:', self.grouping)

        self.group_size = len(self.grouping)
        # use HumanBody.imubone as bone_vectors dict key
        self.imubone_mapping = {
            # 'Head': 8,
            'Pelvis': 2, 'L_UpArm': 11, 'R_UpArm': 13, 'L_LowArm': 12, 'R_LowArm': 14,
    ...
    def get_group(self, db):
        ...
        for i in range(nitems):
            ...
            if keystr not in grouping:
                ...
                grouping[keystr] = [-1, -1] # 有幾台相機就要有幾個-1
            grouping[keystr][camera_id] = i # 將當前條目的索引i存儲在對應於當前關鍵字字符串和camera_id的位置
        # print('=>grouping:', grouping)

        filtered_grouping = []
        for _, v in grouping.items():
            if np.all(np.array(v) != -1):   # 檢查當前條目的值（一個列表）是否所有元素都不等於-1
                filtered_grouping.append(itemgetter(*self.selected_cam)(v)) # 如果所有元素都不等於-1，則使用itemgetter從列表v中獲取self.selected_cam指定的元素，並將其添加到filtered_grouping列表中
    ...
    def __getitem__(self, idx):
        ...
        for item in items:
            ...
            i, m = super().__getitem__(item, source='totalcapture', tc_imubone_map=self.imubone_mapping)  # extra args for data source
    ...
    def evaluate(self, pred, *args, **kwargs):
        ...
        detected_int = detected.astype(int)
