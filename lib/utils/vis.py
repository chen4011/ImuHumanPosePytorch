...
def save_debug_heatmaps(config, input, meta, target, joints_pred, output, prefix):
    if not config.DEBUG.DEBUG:
        return
    ...
    #     save_batch_heatmaps(input, target, '{}_hm_gt.jpg'.format(prefix2))
    if config.DEBUG.SAVE_HEATMAPS_PRED:
        save_batch_fusion_heatmaps(input, output, '{}_hm_pred.jpg'.format(prefix2), normalize=True)
