...
def main():
    ...
    for i, items in enumerate(heatmap_loader):
        ...
        # preds = []
        # maxvs = []
        nview = heatmaps.shape[0]   #相機數量
        njoints = heatmaps.shape[1] #關節數量
        ...
        centers = [boxes[i]['center'] for i in range(nview)]    # 平面照片的中心點
        scales = [boxes[i]['scale'] for i in range(nview)]    # 平面照片的縮放比例
