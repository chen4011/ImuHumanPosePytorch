...
def main():
    ...
    prediction_path = os.path.join(final_output_dir,
                                   config.TEST.HEATMAP_LOCATION_FILE)   # prediction_path: 'output/predicted_heatmaps.h5'
    # prediction_path = os.path.join(final_output_dir, 'image_only_heatmaps.h5')
    logger.info(prediction_path)
    test_dataset = eval('dataset.' + config.DATASET.TEST_DATASET)(
        config, config.DATASET.TEST_SUBSET, False)  # test_dataset = dataset.totalcapture
    ...
    if config.DATASET.TRAIN_DATASET == 'multiview_h36m':
        ...
        for k in pairwise:
            no_sparse_pairwise[k] = pairwise[k].toarray().astype(int)
    ...
    for edge in pairwise:
        edge_pairwise = pairwise[edge].astype(int)
    ...
    p3dpose_align = []
    p3dpose_gt = []
    with open(os.path.join(final_output_dir, '3dpose_prediction.txt'), 'w') as f:
        for i, items in tqdm(enumerate(heatmap_loader)):
            ...
            for idx_datum, prediction in enumerate(outputs_cat):
                ...
                logger.info('idx_{:0>6d} sub_{} act_{} subact_{} done!'.format(
